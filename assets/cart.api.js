        ...
        if (key === 'items') {
          ...
          if (value.length == 1) {
            ...
            jQuery.each(value, function(i, item) {
              ...
              jQuery('<li class="mini-cart-item"><div class="mini-cart-product-img"><a href="' + item.url + '"><img src="' + item.image + '" alt="{{ item.alt }}"></a><span class="product-quantity">' + item.quantity + 'x</span></div><div class="mini-cart-product-desc"><h3><a href="' + item.url + '">' + item.title + '</a></h3><div class="price-box"><span class="new-price">' + Shopify.formatMoney(item.price, formatMoney) + '</span></div></div><div class="remove-from-cart"><a href="javascript:void(0);" onclick="Shopify.removeItem(' + item.variant_id + ')" title="Remove"><i class="fa fa-trash"></i></a></div></li>').appendTo(table);
            });
          }			
